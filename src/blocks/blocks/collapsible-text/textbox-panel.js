...
const TextBoxPanel = ( { attributes, setAttributes } ) => {
	return (
		<>
			<PanelBody
				title={ __( 'Text Box Settings', 'awesome-blocks' ) }
				initialOpen={ true }
			>
				<TextareaControl
				...
				<RadioControl
					...
					options={ [
						{
							label: __( 'Expand', 'awesome-blocks' ),
							value: 'expand',
						},
						{
							label: __( 'Link', 'awesome-blocks' ),
							value: 'link',
						},
					] }
				...
				{ attributes.buttonDo == 'expand' && (
					<>
						<TextControl
							...
							onChange={ ( val ) =>
								setAttributes( { expandButtonText: val } )
							}
						/>
						<TextControl
							label={ __(
							...
							onChange={ ( val ) =>
								setAttributes( { collapseButtonText: val } )
							}
						/>
					</>
				) }
