...
domReady( () => {
	const accordions = document.querySelectorAll( '.wp-block-awesome-blocks-accordion' );
	...
	const addFAQSchema = accordion => {
		...
		items.forEach( item => {
			const questionElem = item.querySelector( ':scope > .wp-block-awesome-blocks-accordion-item__title > *' );
			const question = questionElem?.innerText ?? questionElem?.innerHTML?.replace( /<[^>]*>?/gm, '' );
			const textElements = item.querySelectorAll( ':scope > .wp-block-awesome-blocks-accordion-item__content :is(p, h1, h2, h3, h4, h5, h6)' );
