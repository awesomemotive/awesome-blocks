...
domReady( function () {
	const tabs = document.querySelectorAll( '.wp-block-awesome-blocks-tabs' );

	tabs.forEach( tab => {
		const items = Array.from( tab.querySelectorAll( ':scope > .wp-block-awesome-blocks-tabs__content > .wp-block-awesome-blocks-tabs-item' ) );
		const header = document.createElement( 'div' );
		header.classList.add( 'wp-block-awesome-blocks-tabs__header' );
		tab.prepend( header );
		...
		items.forEach( ( item, index ) => {
			const content = item.querySelector( ':scope > .wp-block-awesome-blocks-tabs-item__content' );
			content.setAttribute( 'role', 'tabpanel' );

			const headerMobile = item.querySelector( ':scope > .wp-block-awesome-blocks-tabs-item__header' );
			headerMobile.setAttribute( 'role', 'tab' );

			const headerItem = document.createElement( 'div' );
			headerItem.classList.add( 'wp-block-awesome-blocks-tabs__header_item' );
			headerItem.tabIndex = 0;

			const tabTitle = headerMobile.cloneNode( true );
			tabTitle.classList.remove( 'wp-block-awesome-blocks-tabs-item__header' );
			...
			const toggleTabs = ( i, idx ) => {
				const content = i.querySelector( ':scope > .wp-block-awesome-blocks-tabs-item__content' );
				const headerMobile = i.querySelector( ':scope > .wp-block-awesome-blocks-tabs-item__header' );
