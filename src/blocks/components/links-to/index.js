...
const LinksTo = ( { attributes, setAttributes } ) => {
	...
	if ( attributes.linksTo == 'anchor' && attributes.linkToPage > 0 ) {
		let post = wp.data
			.select( 'core' )
			.getEntityRecord( 'postType', 'page', attributes.linkToPage );
		if ( post?.content?.raw ) {
			let parser = new DOMParser();
			let htmlParsed = parser.parseFromString(
				post.content.raw,
				'text/html'
			);
		...
		<>
			...
			<RadioControl
				...
				options={ [
					...
					{
						label: __( 'Web address', 'awesome-blocks' ),
						value: 'web_address',
					},
					{
						label: __( 'Anchor', 'awesome-blocks' ),
						value: 'anchor',
					},
					{
						label: __( 'Top/bottom of page', 'awesome-blocks' ),
						value: 'top_bottom_of_page',
					},
					{
						label: __( 'Document', 'awesome-blocks' ),
						value: 'document',
					},
					{ label: __( 'email', 'awesome-blocks' ), value: 'email' },
					{
						label: __( 'Phone number', 'awesome-blocks' ),
						value: 'phone_number',
					},
				] }
				onChange={ ( val ) => {
			...
			{ attributes.linksTo == 'page' && (
				<>
					<SelectControl
						...
						options={ pageOptions }
						onChange={ ( val ) =>
							setAttributes( { linkToPage: Number( val ) } )
						}
					/>
					<RadioControl
						...
						options={ [
							{
								label: __( 'New window', 'awesome-blocks' ),
								value: 'new',
							},
							{
								label: __( 'Current window', 'awesome-blocks' ),
								value: 'current',
							},
						] }
						onChange={ ( val ) =>
							setAttributes( { linksOpen: val } )
						}
					/>
				</>
			) }
			...
			{ attributes.linksTo == 'anchor' && (
				<>
					...
					/>
					{ attributes.linkToPage > 0 && (
						<SelectControl
							label={ __(
								'Which anchor on this page?',
								'awesome-blocks'
							) }
			...
			{ attributes.linksTo == 'top_bottom_of_page' && (
				<RadioControl
					label={ __(
						"Link visitors to the top/bottom of the page (whichever page they're on).",
						'awesome-blocks'
					) }
					...
					options={ [
						...
						{
							label: __( 'Bottom', 'awesome-blocks' ),
							value: 'bottom',
						},
					] }
					onChange={ ( val ) =>
						setAttributes( { topBottomOfThePage: val } )
					}
				/>
			) }
						...
						render={ ( { open } ) => (
							<BaseControl>
								<BaseControl.VisualLabel>
									{ __(
										'What doc are you linking to?',
										'awesome-blocks'
									) }
								</BaseControl.VisualLabel>
								<Button onClick={ open } variant="secondary">
									{ __( 'Choose File', 'awesome-blocks' ) }
								</Button>
							</BaseControl>
						) }
					/>
			...
			{ attributes.linksTo == 'email' && (
				<>
					<InputControl
						...
						onChange={ ( val ) =>
							setAttributes( { emailAddress: val ?? '' } )
						}
					/>
					<TextControl
						label={ __(
			...
			{ attributes.linksTo == 'phone_number' && (
				<TextControl
					label={ __(
						'Add your number with country & area code',
						'awesome-blocks'
					) }
					placeholder={ __(
						'e.g., +1-234-567890',
					...
					help={ __(
						'Add your country code and area code so customers anywhere in the world can be in touch.',
						'awesome-blocks'
					) }
